...
- Pushed 7 commits; deploy verified in 3 min via the eor-migration hero marker.
- Live form tests on production: header 6935 (employer → Clients admin + auto-reply), footer 6936 (job seeker → Candidates admin only), popup 6937 (employer → correct 2 emails), lead-input 6938. All Zapier hooks fired from the live server; Codi to confirm the 4 runs in Zapier history.
- WP-side Zapier feeds 48–54 (forms 24/28/29/31) all deactivated and confirmed inactive — double-send risk closed. Form 26's old feed left untouched.

### 2026-07-20 (latest 3) — Marketing subheadings for 9 service pages *(live)*
...
### 2026-07-20 (latest 2) — All Zap hooks wired + GF email routing live *(live)*
...
### 2026-07-20 (latest) — Direct server→Zapier delivery (option 2) *(live)*
...
- A/B delivery test submitted 13:26 UK: form 26 entry 6929 (old Zap's own form) and form 29 entry 6930 (live-site path). Codi checked: ZERO runs — even his fresh, properly-subscribed Zap got nothing.
- Diagnosis: WP delivers nothing to Zapier from ANY form while inline email notifications succeed (entry notes prove it) → GF's asynchronous feed delivery (background processor) is broken on the WP host. Affects header/footer/popup and old form 26 alike. WP-side fix needs wp-admin (GF System Status → background tasks; likely loopback requests blocked by CleanTalk/Cloudflare/host).
- Proved the escape hatch: direct POST of a GF-shaped payload (field-label keys) to Codi's new Zap hook → Zapier accepted (status success) → run should appear in history. Proposal: our Nitro server posts to the Zap hook directly after GF success (could also cover the feed-less guide form); deactivate the WP feeds when switching to avoid future double-sends.
